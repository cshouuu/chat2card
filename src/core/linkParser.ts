 ...
 * 分享链接解析器。
 *
 * 纯前端无法直接跨域抓取分享页,这里借助 Jina Reader(r.jina.ai)
 * 在服务端抓取目标页面并转为 markdown——该服务返回动态 CORS 头,
 * 因此浏览器可以直接 fetch。对话内容会经第三方服务中转,
 * 敏感对话请使用粘贴模式。
 */
...
export const PLATFORM_INFO: Record<SharePlatform, { name: string; support: 'full' | 'best-effort' | 'blocked' }> = {
  ...
  chatgpt: { name: 'ChatGPT', support: 'blocked' },
  gemini: { name: 'Gemini', support: 'blocked' },
 ...
 * DeepSeek 分享页(尽力解析)。
 * Jina 输出没有角色标记,结构大致为:
 *   [页面头部文案] [用户消息] [Thought for N seconds] [思考内容] [AI 回答] ...
 * 这里取:用户消息 = 头部之后到第一个 "Thought" 之前;
 * AI 回答 = 第一个 "Thought" 之后的内容(思考过程一并保留,尽力模式)。
 */
export function parseDeepSeekMarkdown(md: string): ParsedChat {
  const body = stripJinaHeader(md).trimStart();
  // 循环剔除开头的固定页面文案(可能出现多组)
  let cleaned = body;
  const headPatterns = [
...
/**
 * 豆包分享页(尽力解析)。
 * Jina 输出中,页面标题通常就是第一条用户问题,正文是 AI 回答。
 */
...
export async function parseShareLink(url: string): Promise<ParsedChat> {
  ...
  switch (platform) {
    ...
    case 'chatgpt':
      throw new ParseError(
        'ChatGPT 分享链接需要登录后才能查看,暂无法自动解析。\n请在 ChatGPT 中导出对话 JSON(设置 → 数据控制 → 导出),或直接复制文本粘贴到本工具。',
      );
    case 'gemini':
      throw new ParseError(
        'Gemini 分享页暂无法自动解析(平台反爬)。\n请直接复制对话文本粘贴到本工具。',
      );
    default:
